...
// Constants
var WHICH_COMMAND_NOT_FOUND_CODE = 1;
...
Lambda.prototype._params = function (program, buffer) {
  var params = {
    ...
    VpcConfig: {},
    IncludeNodeModules: program.includeNodeModules,
    FollowSymlinks: program.followSymlinks
  };
  if (program.version) {
...
Lambda.prototype._rsync = function (program, codeDirectory, callback) {
  ...
  // Base command
  var rsyncCommand = ['rsync', '-r'];

  // Flow symlinks if specified
  if (program.followSymlinks) {
    rsyncCommand.push('-L');
  }
  ...
  excludeGlobs
    ...
    .forEach(function(excludeParam) {
      rsyncCommand.push(excludeParam);
    });
  ...
  rsyncCommand.push(codeDirectory);
  ...
  exec('mkdir -p ' + codeDirectory, function(err) {
    ...
    exec(rsyncCommand.join(' '), function (err) {
      if (err) {
        return callback(err);
      }
...
Lambda.prototype._npmInstall = function (program, codeDirectory, callback) {
  if (program.includeNodeModules) {
    console.log('=> Skipping node install because node modules have been included. (See --includeNodeModules switch)');
    return callback(null, true);
  }
...
Lambda.prototype._nativeZipInstalled = function (cb) {
  // The 'which' command will return success if zip is available or 1 if not
  // An error code other than 1 means that something else went wrong so the error should
  // be handled.
  exec('which zip', function(err) {
    if (err) {
      if (err.code === WHICH_COMMAND_NOT_FOUND_CODE) {
        cb(false);
      } else {
        cb(null, err);
      }
    ...
    _this._rsync(program, codeDirectory, function (err) {
      ...
      _this._npmInstall(program, codeDirectory, function (err) {
        ...
        _this._postInstallScript(codeDirectory, function (err) {
          ...
          _this._nativeZipInstalled(function (err, nativeZipAvailable) {
            if (err) {
              return archive_callback(err);
            }
            if (!nativeZipAvailable) {
              console.warn('=> Warning: Could not find the zip utility in the local path. Falling back to js version');
            }
            var shouldUseNativeZip = process.platform !== 'win32' && nativeZipAvailable;
            var archive = shouldUseNativeZip ? _this._nativeZip : _this._zip;
